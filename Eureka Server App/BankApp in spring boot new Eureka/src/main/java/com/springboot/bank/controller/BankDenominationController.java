...
public class BankDenominationController {
	...
	public ResponseEntity<Denomination> getTotalDenominationForBank(final @RequestBody WrapperDenomination wrapperDenomination ) throws BankException {
		...
		try {
			bankDenominationService.addDenominationNew(wrapperDenomination.getAmount(),wrapperDenomination.getBankId());
 ...
 */
@PostMapping("/") public ResponseEntity<Denomination> getTotalDenominationForATM(final @RequestBody WrapperDenomination wrapperDenomination ) throws BankException {
	final Denomination bankDenomination = new Denomination();
		try {
			bankDenominationService.addDenominationNew(wrapperDenomination.getAmount(),wrapperDenomination.getAtmID());
		} catch (BankException e) {
			System.out.println(e.getMessage());
			LOGGER.info(e.getMessage());
		}
